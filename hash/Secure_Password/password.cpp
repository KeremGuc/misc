...
public:
    ...
    bool changePassword(const std::string& username, const std::string oldPassword, const std::string& newPassword) {
        if(!verifyPassword(username, oldPassword)) {
            return false; //old password doesnt match
        }
        int index = findIndex(username);
        int i = 0;
        while(i < size) {
            ...
            if(!entries[currentIndex].isDeleted && entries[currentIndex].username == username) {
                std::string newPasswordHash = hashPassword(newPassword, entries[currentIndex].salt);
                if(newPasswordHash == entries[currentIndex].passwordHash) {
                    return false; //new password is the same as old one
                }
                break; //user found
            }
            ++i;
        }
        return storePassword(username, newPassword); //store new password
    }
...
int main() {
    ...
    switch(choice) {
        ...
        std::string oldPassword, newPassword;
            ...
            if(!storage.verifyPassword(currentuser, oldPassword)) {
            ...
            while(!passwordMatch) {
                ...
                if (!storage.strengthChecker(newPassword)) {
                    continue;
                }
            ...
            if (storage.changePassword(currentuser, oldPassword, newPassword)) {
                std::cout << "Password changed successfully!" << std::endl;
            } else {
                std::cout << "New password cant be the same as old password." << std::endl;
                std::cout << "=================================" << std::endl;
            }
